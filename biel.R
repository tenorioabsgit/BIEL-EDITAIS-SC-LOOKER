...
id <- list.files(path = 'arquivos_downloaded/pdfs/', pattern = 'pdf', full.names = TRUE)
...
for (i in 1:nrow(base_final_lista_download)) {
  ...
  repeat {
    if (str_detect(string = base_final_lista_download$link_completo_para_editais[i], pattern = 'pdf')) {
      ...
      tmp <- try(download.file(url = base_final_lista_download$link_completo_para_editais[i],
                        destfile = paste0('/home/pira/google_drive/ciencia_de_dados/R/DOU_SC/arquivos_downloaded/docs/',
                                          base_final_lista_download$id[i],
                                          '.doc')))}
...
#### Lista de Itens
#### o código abaixo carrega a lista de materiais em uma fonte PDF e já a
...
# base_final_itens$itens <- str_remove_all(string = base_final_itens$itens, pattern = '[[:punct:]]')
# 
# base_final_itens <- base_final_itens %>% arrange(itens)
# base_final_itens <- base_final_itens %>% distinct(itens, .keep_all = TRUE)
# base_final_itens <- base_final_itens %>% filter(!is.na(itens))
# base_final_itens <- base_final_itens %>% filter(itens != "")
# 
# base_termos_final <- data.frame()
# for (i in 1:nrow(base_final_itens)) {
#   item <- as.data.frame(base_final_itens$itens[i])
#   item <- item %>% rename('itens' = 'base_final_itens$itens[i]')
#   print(paste0('Separando coluna ',i, ' de ', nrow(base_final_itens)))
#   base_termos_provisorio <- cSplit(item, 'itens', sep = ' ')
#   base_termos_provisorio <- as.data.frame(t.data.frame(base_termos_provisorio))
#   rownames(base_termos_provisorio) <- NULL
#   base_termos_provisorio <- base_termos_provisorio %>% mutate_all(as.character)
#   base_termos_final <- bind_rows(base_termos_final, base_termos_provisorio)
# }
# 
# base_termos_final <- base_termos_final %>% rename('itens' = 'V1')
# base_termos_final <- base_termos_final %>% arrange(itens)
# base_termos_final <- base_termos_final %>% distinct(itens, .keep_all = TRUE)
# base_termos_final <- base_termos_final %>% filter(!is.na(itens))
# base_termos_final <- base_termos_final %>% filter(itens != "")
# 
# write.csv(base_termos_final, 'backup_bases/base_termos_final.csv')
# base_final_itens <- base_termos_final
# 
# rm(base_termos_final)
# rm(base_termos_provisorio)
# 
# base_final_itens <- base_final_itens$itens
# 
# lista_string_final <- " "
